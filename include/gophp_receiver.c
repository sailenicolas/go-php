...
static int call_receiver_method(INTERNAL_FUNCTION_PARAMETERS) {
    zval args;
    array_init_size(&args, ZEND_NUM_ARGS());
    if (zend_copy_parameters_array(ZEND_NUM_ARGS(), &args) == FAILURE) {
        RETVAL_NULL();
    } else {
        gophp_object *result = engineReceiverCall(Z_OBJ_P(getThis()), ZSTR_VAL(EX(func)->common.function_name),
                                                  (void *) &args);
        if (result == NULL) {
            RETVAL_NULL();
        } else {
...
static void receiver_method_handler(INTERNAL_FUNCTION_PARAMETERS) {
    /* Cleanup trampoline */
    call_receiver_method(INTERNAL_FUNCTION_PARAM_PASSTHRU);
...
void receiver_define(char *name) {
    ...
    *class_entry = orig_class_entry;
    ...
    if (class_entry->info.internal.builtin_functions) {
        zend_register_functions(class_entry, class_entry->info.internal.builtin_functions, &class_entry->function_table,
                                EG(current_module)->type);
    }

    class_entry->create_object = init_receiver;
    class_entry->ce_flags =
            orig_class_entry.ce_flags | ZEND_ACC_CONSTANTS_UPDATED | ZEND_ACC_LINKED | ZEND_ACC_RESOLVED_PARENT |
